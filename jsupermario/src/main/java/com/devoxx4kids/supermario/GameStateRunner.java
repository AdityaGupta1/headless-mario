...
public class GameStateRunner {
    ...
    static GameState runGameStateSingleFrame(GameState gs, InputState is, WorldWrapper world) {
        i++;
        List<Enemy> enemiesActive = handleEnemies(gs, world);
        ...
        handlePlayerHorizontalMovement(is, world, player);
            ...
            if (player.getVerticalAcc() > 0) {
                ...
                for (int i = 1; i <= player.getVerticalAcc(); i++) {
                    if (world.get(player.getPlayerYPosition() - 1, player.getPlayerXPosition()) != 0 || world.get(player.getPlayerYPosition()- 1,player.getRightX()) != 0) {
                ...
                for (int i = 1; i > player.getVerticalAcc(); i--) {
                    if (world.get(player.getBottomY() + 1, player.getPlayerXPosition()) != 0 || world.get(player.getBottomY() + 1, player.getRightX()) != 0) {
                        player.setVerticalAcc(0);
                        if(!is.isJump())
      ...
      static double getFallRate(boolean isJumpPressed, Player p ){
        if (isJumpPressed){
            if(p.getJumpSpeed() <= 1) {
                 return 0.125;
            }
    ...
    private static void handlePlayerHorizontalMovement(InputState is, WorldWrapper world, Player player) {
        if (is.isRigthPressed()) {
            player.setHorizontalAcc(Math.min(MAX_HORIZONTAL, Math.max(HORIZONTAL_MIN_SPEED ,HORIZONTAL_MOVE_PER_BUTTON + player.getHorizontalAcc())));
        } else if (is.isLeftPressed()) {
            player.setHorizontalAcc(Math.max(-MAX_HORIZONTAL,Math.min(-HORIZONTAL_MIN_SPEED,  -HORIZONTAL_MOVE_PER_BUTTON +  player.getHorizontalAcc())));
        }
        ...
        if (player.getHorizontalAcc() > 0) {
            ...
            for (int i = 0; i > player.getHorizontalAcc(); i--) {
                if (player.getPlayerXPosition() > 0 && world.get(player.getPlayerYPosition(),player.getPlayerXPosition() - 1) == 0
    ...
    private static List<Enemy> handleEnemies(GameState gs, WorldWrapper world) {
        Player player = gs.getPlayer();
        gs.getActiveEnemy().stream()
          ...
          gs.getActiveEnemy().stream().filter(enemy -> enemy != null ).forEach(enemy -> moveEnemy(world, enemy,gs));
        ...
        gs.getInactiveEnemy().stream().filter(e -> e.getPosX() - player.getPlayerXPosition() < ENEMY_ACTIVATION_THRESHOLD)
                .forEach(e -> {gs.getActiveEnemy().add(e);  gs.getEnemiesToRemoveFromAdded().add(e);});
    ...
    private static void moveEnemy(WorldWrapper world, Enemy enemy,GameState gs) {
        ...
        for (int i = 0; i < ENEMY_MOVEMENT_SPEED; i++) {
            ...
            if (world.get(enemy.bottomEdgeY() + 1, enemy.getPosX()) == 0 &&
                ...
                if (world.get(enemy.posY(), (enemy.posX()) + (enemy.direction() == -1 ? -1 : enemy.WIDTH + 1)) == 0) {
