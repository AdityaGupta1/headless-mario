  ...
  private void evaluateSprites() {
    ...
    if (scanlineCycle == 0) {
      ...
      if (scanlineCycle == 257) {
        ...
        out1: if (noSpriteLimit) {
          out2: if (spriteCount == 32) {  
  ...
  private void executeCycle() {
    ...
    if (!rendering && (writeDelay | readDelay) == 0) {
      ...
      if (!rendering && (V & 0x3F00) == 0x3F00) {
        paletteIndex = grayscale & paletteRAM[V & 0x1F];
      } else if ((tileBitmap & 3) != 0) {
        paletteIndex = grayscale & paletteRAM[tileBitmap & 0x1F];
      } else {
        paletteIndex = grayscale & paletteRAM[0];
      } 
      ...
      screen[screenIndex++] = emphasis 
          | (invertPalette ? INVERSE_PALETTE[paletteIndex] : paletteIndex);
		}
