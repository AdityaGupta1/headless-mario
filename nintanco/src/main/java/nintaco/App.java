...
public final class App {
  ...
  public static void init(final String... args) throws Throwable {    
    ...
    EventQueue.invokeAndWait(() -> {
      ...
      imageFrame.init();
      imageFrame.getImagePane().setTVSystem(PAL);
  ...
  private static void loadLicense() {
    ...
    try(final BufferedReader br = new BufferedReader(new InputStreamReader(
          App.class.getResourceAsStream("/nintaco/lgpl-2.1.txt")))) {
      String line = null;
  ...
  private static void setDiskActivity(final boolean activity, 
    ...
    switch(indicator) {
      case NUM_LOCK:
        Toolkit.getDefaultToolkit().setLockingKeyState(VK_NUM_LOCK, activity);
        break;
      case CAPS_LOCK:
        Toolkit.getDefaultToolkit().setLockingKeyState(VK_CAPS_LOCK, activity);
        break;
      case SCROLL_LOCK:
        Toolkit.getDefaultToolkit().setLockingKeyState(VK_SCROLL_LOCK, 
            activity);
        break;
    }
  }
  ...
  public static boolean loadFdsBIOS() {
    ...
    try {
      final FilePath filePath = AppPrefs.getInstance()
          .getFamicomDiskSystemPrefs().getBiosFile();
      if (filePath == null) {
        return false;
      }
  ...
  public static Mapper loadFile(final DataInputStream in, final long fileSize, 
    ...
    switch(fileType) {
      case FDS:
        nesFile = null;      
        fdsFile = new FdsFile(in, fileSize, entryFileName, archiveFileName, 
            fdsBIOS);
        unifFile = null;
        nsfFile = null;
